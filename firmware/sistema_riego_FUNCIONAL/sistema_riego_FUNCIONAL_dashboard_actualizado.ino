...
//    → Datos enviados al dashboard personalizado (Railway + Aiven)
//    → ThingSpeak se mantiene como respaldo de historial
...
// ── DASHBOARD PERSONALIZADO (RAILWAY + AIVEN) ─────────────
// Página pública desarrollada por el equipo.
const char* DASHBOARD_URL =
  "https://taller-de-diseno-production.up.railway.app";

// Endpoint que recibe las mediciones y las guarda en PostgreSQL (Aiven).
const char* DASHBOARD_API_URL =
  "https://taller-de-diseno-production.up.railway.app/api/mediciones";

// Debe ser EXACTAMENTE la misma DEVICE_API_KEY configurada en Railway.
// No subas el archivo con la clave real a un repositorio público.
const char* DEVICE_API_KEY = "fortnite2025";
...
void setup() {
  ...
  Serial.println("[SISTEMA] Dashboard: " + String(DASHBOARD_URL));
}
...
void loop() {
  ...
  if (ahora - ultimaLecturaSensor >= INTERVALO_SENSOR) {
    ...
    // Enviar datos a ThingSpeak como respaldo de historial
    enviarAThingSpeak(humedad, esSeco ? 0 : 1, lluviaProxima24h);

    // Enviar los mismos datos al dashboard personalizado.
    // La API de Railway los guarda en PostgreSQL de Aiven.
    enviarADashboard(
      humedad,
      rawADC,
      esSeco,
...
void enviarAlertaTelegram(float humedad, int rawADC) {
  ...
  mensaje += "📈 *Ver dashboard e historial:*\n";
  mensaje += "   [Dashboard personalizado](" + String(DASHBOARD_URL) + ")\n";
  mensaje += "──────────────────";
...
void enviarRecordatorioTelegram(float humedad) {
  ...
  mensaje += "\n📈 [Ver historial](" + String(DASHBOARD_URL) + ")";
...
void enviarConfirmacionRiego(float humedad) {
  ...
  mensaje += "📈 [Ver historial](" + String(DASHBOARD_URL) + ")";

  bot.sendMessage(TELEGRAM_CHAT_ID, mensaje, "Markdown");
}

// ============================================================
//  FUNCIONES DE SENSOR, DATOS Y DASHBOARD
// ============================================================
...
/**
 * enviarADashboard(humedad, rawADC, esSeco, temperatura, lluvia)
 * Envía una medición en formato JSON a la API propia alojada en Railway.
 * Railway valida X-API-Key y guarda los datos en PostgreSQL de Aiven.
 */
void enviarADashboard(
  float humedad,
  int rawADC,
  bool esSeco,
  float temperatura,
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[Dashboard] ❌ Sin WiFi. No se enviaron datos");
    return;
  }

  // Evita enviar por error la clave de ejemplo.
  if (String(DEVICE_API_KEY).startsWith("PEGA_AQUI")) {
    Serial.println("[Dashboard] ❌ Falta configurar DEVICE_API_KEY");
    return;
  }

  // Cliente HTTPS independiente para no interferir con Telegram.
  WiFiClientSecure clienteDashboard;
  clienteDashboard.setInsecure();

  HTTPClient http;
  http.setTimeout(12000);

  if (!http.begin(clienteDashboard, DASHBOARD_API_URL)) {
    Serial.println("[Dashboard] ❌ No se pudo iniciar la conexión HTTPS");
    return;
  }
  ...
  doc["humedad"]     = humedad;
  doc["adc"]         = rawADC;
  doc["suelo_seco"] = esSeco;
  doc["temperatura"] = temperatura;
  doc["lluvia_24h"]  = lluvia;
  doc["dispositivo"] = "ESP8266-NodeMCU";

  String cuerpoJSON;
  serializeJson(doc, cuerpoJSON);

  int codigoHTTP = http.POST(cuerpoJSON);

  if (codigoHTTP == 201) {
    Serial.println("[Dashboard] ✅ Medición guardada en Aiven");
  } else {
    Serial.println("[Dashboard] ❌ Error HTTP: " + String(codigoHTTP));

    // Mostrar la respuesta ayuda a diagnosticar 400, 401 o 500.
    if (codigoHTTP > 0) {
      String respuesta = http.getString();
      if (respuesta.length() > 0) {
        Serial.println("[Dashboard] Respuesta: " + respuesta);
      }
    }
  }
